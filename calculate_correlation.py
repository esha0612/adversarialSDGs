...
class CorrelationAnalyzer:
    ...
    def extract_responses(self, all_game_data):
        ...
        for idx, game in enumerate(all_game_data):
            ...
            table_talk = game.get('table_talk', [])
            print(f"[DEBUG] Game {idx} table_talk length: {len(table_talk)}")
            for t_idx, talk in enumerate(table_talk):
                player_name = talk.get('speaker', None)
                text = talk.get('text', None)
                print(f"[DEBUG]     TableTalk {t_idx} keys: {list(talk.keys())}, text present: {text is not None}")
                if player_name and text:
                    responses.append({
                        'game_id': game_id,
                        'player_name': player_name,
                        'response_type': 'table_talk',
                        'response_text': text,
                    })
        print(f"Extracted {len(responses)} table_talk responses from all games")
        return responses

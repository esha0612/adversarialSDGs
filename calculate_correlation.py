...
class CorrelationAnalyzer:
    ...
    def extract_responses(self, all_game_data):
        ...
        - discussion_statements: Each player's LLM-generated discussion statement
        ...
        for idx, game in enumerate(all_game_data):
            ...
            mission_phases = game.get('mission_phases', [])
            print(f"[DEBUG] Game {idx} keys: {list(game.keys())}")
            print(f"[DEBUG] Game {idx} mission_phases length: {len(mission_phases)}")
            for m_idx, mission in enumerate(mission_phases):
                discussion_statements = mission.get('discussion_statements', [])
                print(f"[DEBUG]   Mission {m_idx} discussion_statements length: {len(discussion_statements)}")
                for sidx, statement in enumerate(discussion_statements):
                    player_name = statement.get('player_name', None)
                    llm_response = statement.get('llm_response', None)
                    print(f"[DEBUG]     Statement {sidx} keys: {list(statement.keys())}, llm_response present: {llm_response is not None}")
                    if player_name and llm_response:
                        responses.append({
                            'game_id': game_id,
        ...
        print(f"Extracted {len(responses)} discussion statement responses from all games")
        return responses
    ...
    def run_full_analysis(self):
        ...
        # Save results to CSV
        print("\n=== Saving Results ===")
        valid_df.to_csv('correlation_analysis_aggregated_user_branch3.csv', index=False)
        print("Aggregated results with user values saved to: correlation_analysis_aggregated_user_branch3.csv")
        ...
        summary_df.to_csv('correlation_analysis_summary_user_branch3.csv', index=False)
        print("Summary results saved to: correlation_analysis_summary_user_branch3.csv")

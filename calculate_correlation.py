...
class CorrelationAnalyzer:
    ...
    def __init__(self, game_records_dir):
        """
        Initialize the analyzer.
        
        Args:
            game_records_dir: Path to directory containing JSON game records
        """
    ...
    def load_game_records(self):
        ...
        """
        game_files = list(self.game_records_dir.glob("*.json"))
        print(f"Found {len(game_files)} game record files")
        ...
        print(f"Successfully loaded {len(all_game_data)} game records")
    ...
    def extract_responses(self, all_game_data):
        ...
        - play_reason: The LLM's reasoning for their card play
        - behavior: The LLM's behavioral description
        - challenge_reason: The LLM's reasoning for challenging (if applicable)
        ...
        - response_type: Type of response (play_reason, behavior, challenge_reason)
        ...
        for game in all_game_data:
            ...
            # Extract player names (represents different models)
            ...
            for round_data in game.get('rounds', []):
                ...
                # Extract responses from play history
                for play_entry in round_data.get('play_history', []):
                    player_name = play_entry.get('player_name', 'unknown')
                    
                    # Extract play reason
                    play_reason = play_entry.get('play_reason', '')
                    if play_reason:
                        responses.append({
                            'game_id': game_id,
                            'player_name': player_name,
                            'round_id': round_id,
                            'response_type': 'play_reason',
                            'response_text': play_reason,
                        })
                    
                    # Extract behavior description
                    behavior = play_entry.get('behavior', '')
                    if behavior:
                        responses.append({
                            'game_id': game_id,
                            'player_name': player_name,
                            ...
                            'response_text': behavior,
                        })
                    
                    # Extract challenge reason
                    challenge_reason = play_entry.get('challenge_reason', '')
                    if challenge_reason:
                        responses.append({
                            'game_id': game_id,
                            'player_name': player_name,
                            'round_id': round_id,
                            'response_type': 'challenge_reason',
                            'response_text': challenge_reason,
                        })
        
        print(f"Extracted {len(responses)} individual responses")
        return responses
...
def main():
    ...
    analyzer = CorrelationAnalyzer(game_records_dir)
